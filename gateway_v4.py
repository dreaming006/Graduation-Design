...
mqtt_q = Queue(1)
lock = threading.Lock()
event=threading.Event()   #用于从服务器接收publish的事件
...
class MQTT():
    ...
    #From App To Gateway  MQTT客户端网络循环 
    def MQTT_FATG(self):
        self.client.loop_forever()
               
    #From Gateway To App  GateToApp_Queue非空，网关发布消息到APP(topic1 = "Catstause")       
    def MQTT_FGTA(self):
        global GTOA_Q
        while True:
        ...
        if event.isSet():      #接收到服务器的publish
            ...
            if ATOG_Q_num > 0:          
                ...
                try:
                    ...
                    if app_data[1:3] == '90':       
                        GTOL_Q.queue.clear()        #清空树莓派发送到lora的队列
                        GTOL_Q.put(app_data)        #将APP数据转发到lora
                        lora_data = LTOG_Q.get()    #从LORA接收的应答数据
                        print('stop lora_data',lora_data)
                        GTOA_Q.put(lora_data)       #将lora应答数据转发给APP
            ...
            if ATOG_Q_num != 0:
                ...
                try:
                    ...
                    if app_data[1:3] == '98':
                        ...
                        GTOL_Q.put(data)            #树莓派=>lora
                        lora_data = LTOG_Q.get()    #lora=>树莓派
                        nowdata_dict['A'] = lora_data
                        GTOA_Q.put(lora_data)       #树莓派=>APP
                        event.clear()
                        lock.release()
                        print('auto')
                        Auto_run()
