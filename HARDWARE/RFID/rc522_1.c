...
void RC522_1_Init ( void )
{
	RFID_NUM=1;
	SPI1_Init(); 				  				//初始化SPI1
	RC522_1_Reset_Disable();	    //将RST置高,启动内部复位阶段;
	PcdReset_1 ();                  //复位RC522 
	PcdAntennaOff_1();              //关闭天线
	RC522_DELAY();                //delay 2us
	PcdAntennaOn_1();               //开启天线
	M500PcdConfigISOType_1 ( 'A' ); //设置工作模式
	PcdAntennaOff_1();
}
...
void PcdReset_1 ( void )
{
    ...
    delay_us ( 1 );
    WriteRawRC_1 ( ModeReg, 0x3D );                //定义发送和接收常用模式 和Mifare卡通信,CRC初始值0x6363
    WriteRawRC_1 ( TReloadRegL, 30 );              //16位定时器低位   
    WriteRawRC_1 ( TReloadRegH, 0 );			     //16位定时器低位   
    WriteRawRC_1 ( TModeReg, 0x8D );				 //定义内部定时器的设置
    WriteRawRC_1 ( TPrescalerReg, 0x3E );			 //定义定时器分频系数
    WriteRawRC_1 ( TxAutoReg, 0x40 );				 //?调制发送信号为100%ASK		
}
...
char PcdComMF522_1 ( u8 ucCommand, u8 * pInData, u8 ucInLenByte, u8 * pOutData, u32 * pOutLenBit )		
{
    ...
    WriteRawRC_1 ( ComIEnReg, ucIrqEn | 0x80 );		//IRqInv置位管脚IRQ与Status1Reg的IRq位的值相反 
    ClearBitMask_1 ( ComIrqReg, 0x80 );			//Set1该位清零时,CommIRqReg的屏蔽位清零
    WriteRawRC_1 ( CommandReg, PCD_IDLE );		//写空闲命令
    SetBitMask_1 ( FIFOLevelReg, 0x80 );			//置位FlushBuffer清除内部FIFO的读和写指针以及ErrReg的BufferOvfl?标志位被清楚
    
    for ( ul = 0; ul < ucInLenByte; ul ++ )
		WriteRawRC_1 ( FIFODataReg, pInData [ ul ] );    		//写数据进FIFOdata
			
    WriteRawRC_1 ( CommandReg, ucCommand );					//写命令
   
    
    if ( ucCommand == PCD_TRANSCEIVE )
			SetBitMask_1(BitFramingReg,0x80);  				//StartSend置位启动数据发送 该位与收发命令使用时才有效
    
    ul = 1000;//根据时钟频率调整,操作M1卡最大等待时间25ms（可调）
		
    do 														//认证与寻卡等待时间	
    {
         ucN = ReadRawRC_1 ( ComIrqReg );							//查询事件中断
         ul --;
    } while ( ( ul != 0 ) && ( ! ( ucN & 0x01 ) ) && ( ! ( ucN & ucWaitFor ) ) );		//退出条件i=0,定时器中断,与写空闲命令
		
    ClearBitMask_1 ( BitFramingReg, 0x80 );					//清理允许StartSend位
		
    if ( ul != 0 )
    {
		if ( ! (( ReadRawRC_1 ( ErrorReg ) & 0x1B )) )			//读错误标志寄存器BufferOfI CollErr ParityErr ProtocolErr
		{
			cStatus = MI_OK;
			...
			if ( ucCommand == PCD_TRANSCEIVE )
			{
				ucN = ReadRawRC_1 ( FIFOLevelReg );			//读FIFO中保存的字节数
				
				ucLastBits = ReadRawRC_1 ( ControlReg ) & 0x07;	//最后接收到的字节的有效位数
...
char PcdRequest_1 ( u8 ucReq_code, u8 * pTagType )
{
    ...
    ClearBitMask_1 ( Status2Reg, 0x08 );	//清理指示MIFARECyptol单元接通以及所有卡的数据通信被加密的情况
    WriteRawRC_1 ( BitFramingReg, 0x07 );	//发送的最后一个字节的七位
    SetBitMask_1 ( TxControlReg, 0x03 );	//TX1,TX2管脚的输出信号传递经发送调制的13.56的能量载波信号

    ucComMF522Buf [ 0 ] = ucReq_code;		//存入 卡片命令字

    cStatus = PcdComMF522_1 ( PCD_TRANSCEIVE,	ucComMF522Buf, 1, ucComMF522Buf, & ulLen );	//?寻卡  
...
char PcdAnticoll_1 ( u8 * pSnr )
{
    ...
    ClearBitMask_1 ( Status2Reg, 0x08 );		//清MFCryptol On位 只有成功执行MFAuthent命令后,该位才能置位
    WriteRawRC_1 ( BitFramingReg, 0x00);		//清理寄存器 停止收发
    ClearBitMask_1 ( CollReg, 0x80 );			//清ValuesAfterColl所有接收的位在冲突后被清除
   
    ucComMF522Buf [ 0 ] = 0x93;	//?卡片防冲突命令
    ucComMF522Buf [ 1 ] = 0x20;
   
    cStatus = PcdComMF522_1 ( PCD_TRANSCEIVE, ucComMF522Buf, 2, ucComMF522Buf, & ulLen);//与卡片通信
